...
public class ManageUser {
	...
	public boolean isComplete(User user) {
		return (hasValue(user.getName()) && hasValue(user.getSurname()) && hasValue(user.getGender())
				&& hasValue(user.getBirthday()) && hasValue(user.getUid()) && hasValue(user.getEmail())
	...
	public List<User> getUserFollowers(String uid) {
		String query = "SELECT users.uid,users.name,users.avatar FROM users JOIN followers ON users.uid = followers.uid WHERE followers.fid = ?;";
		PreparedStatement statement = null;
		List<User> l = new ArrayList<User>();
		try {
			statement = db.prepareStatement(query);
	...
	// Get all the users
	public List<User> getUnfollowingUsers(String uid) {
		String query = "SELECT U.uid,U.name,U.avatar FROM users AS U WHERE U.uid NOT IN \r\n"
	...
	public List<User> getUsers() {
		String query = "SELECT uid,name,avatar FROM users;";
		PreparedStatement statement = null;
		List<User> l = new ArrayList<User>();
		try {
			statement = db.prepareStatement(query);
